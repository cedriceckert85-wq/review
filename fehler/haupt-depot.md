...
- **Update 21.06.2026 (KW25-Wochen-Review):** Keine neuen Belastungs-Datenpunkte in KW25 — einziger Kauf war XLP (15.06., defensiver Low-Vol-ETF, kein Messer-Test), n bleibt 4. Prozess-Befund der Woche: Die Einstiegs-Bestätigungsregel wurde die **2. Woche in Folge nicht eingeführt** (KW24-PRIO-1 ignoriert). Eintrag bleibt OFFEN; das Leck ist die fehlende Regel, nicht (mehr) ein neuer Daten-Datenpunkt.
- **Status:** OFFEN (n=4, alle 4 Alt-Einstiege erholt aber ex post — Timing-Leck; Messlatte bei n≥8; Wurzel teilt sich mit F-H-004; KW24-Empfehlung 2. Woche ignoriert)
...
- **Update 17.06.2026 (22:30, Schluss):** Der CEG-Test ist entschieden — und negativ: CEG wurde bei Tag 10/10 NICHT exekutiert (Schluss 267,26 €, weiter gehalten). Damit wiederholt sich „Zeit-Exit nicht eingehalten" ein zweites Mal (nach NVDA Tag 9) → **n=2, per Akten-Logik jetzt Gewohnheit, nicht Einzel-Slip**. Diesmal mit Kosten: CEG stand um 17:13 bei 271,35 € (nahe Ziel 273) und fadete bis Schluss auf 267,26 € — die Nicht-Exekution am Zeit-Limit/nahe Ziel gab ~1,5 % Buchgewinn zurück (anders als NVDA, das folgenlos blieb). **Einstufung: SKILL-FEHLER** (wiederkehrende, vermeidbare Disziplinlücke). Messlatte für Abstellung: nächster Zeit-Limit-Treffer wird am Tag X exekutiert oder dokumentiert verlängert.
- **Status:** OFFEN → SKILL-FEHLER (Zeit-Exit-Disziplin, n=2 = Gewohnheit): NVDA Tag 9 + CEG Tag 10/10 beide nicht am Limit exekutiert. Stop-Abstand-Teil bleibt benigne erledigt. Im KW25-Wochen-Review als SKILL-FEHLER festschreiben.
- **Update 18.06.2026 (17:30) — SKILL-Einstufung von gestern war VOREILIG, zurück auf UNKLAR:** CEG wurde am 18.06. zu 275,00 € am ZIEL 273 verkauft (+8,8 %). Das Halten über Tag 10/10 hinaus war also RICHTIG — CEG erholte sich von 267 (Mi-Schluss) auf 275 und traf das Take-Profit. Die Outcomes der Zeit-Exit-Übersteuerungen sind damit GEMISCHT: NVDA (Tag 9, −0,49 %, leicht negativ) vs. CEG (Tag 10→11, +8,8 %, klar positiv). „Immer pünktlich raus" wäre bei CEG falsch gewesen. Der wahre Befund ist also keine verlustträchtige Gewohnheit, sondern eine **inkonsistente, ad-hoc Zeit-Exit-Handhabung** — die Lehre ist eine DOKUMENTIERTE, konsistente Verlängerungsregel (wann Limit hart, wann Ziel-Vorrang), nicht ein starres Limit. (Meta: die Wed-SKILL-Einstufung ist ein Beleg für Reviewer-L10 — voreilige Wertung am Zwischenstand.)
- **Update 21.06.2026 (KW25-Wochen-Review — Konsolidierung):** Wochenbilanz Zeit-Exit: NVDA (Tag 9, +2 Tage spät, −0,49 %) und CEG (über Tag 10/10 gehalten → Ziel 275, +8,8 %) = **gemischte Ergebnisse**. Damit bestätigt sich: ein starres „immer pünktlich raus" wäre bei CEG teuer falsch gewesen, ein „immer halten" bei NVDA leicht negativ. Der Fehler ist nicht das Ergebnis, sondern das **Fehlen einer dokumentierten Entscheidungsregel** (wann Limit hart, wann Ziel-Vorrang). Einstufung bleibt UNKLAR (nicht SKILL — die Wed-SKILL-Einstufung war voreilig, Reviewer-L10). Stop-Abstand-Teil benigne erledigt (L4). Messlatte: beim nächsten Zeit-Limit-Treffer (KW26) wird die Entscheidung dokumentiert begründet — sonst verfestigt sich „inkonsistente Handhabung".
- **Status:** UNKLAR — Zeit-Exit wird ad hoc übersteuert (NVDA leicht negativ, CEG positiv = gemischt); kein klarer SKILL-Fehler. Offen: braucht eine dokumentierte Verlängerungs-/Ziel-Vorrang-Regel. Stop-Abstand-Teil benigne erledigt. KW25 als UNKLAR/Prozess-Konsistenz geführt; nächster Prüfpunkt KW26.
...
- **Update 21.06.2026 (KW25-Wochen-Review):** Kein weiterer Momentum-Chase an einem >+4 %-Tag in KW25 → n bleibt 1, Messlatte (n≥2 in 4 Wochen) nicht getriggert. Eintrag bleibt OFFEN als dokumentierter Einzelfehler; gemeinsame Wurzel mit F-H-002 (Einstieg ohne Bestätigung) unverändert.
- **Status:** OFFEN (n=1 — neu im Wochen-Review KW24 formalisiert; Wurzel teilt sich mit F-H-002; in KW25 kein neuer Chase)
...
| Zeit-Exit ad hoc übersteuert | 2 → UNKLAR (gemischt) | F-H-003 (NVDA Tag 9; CEG Tag 10→11) | NVDA spät → −0,49 % (leicht neg.); CEG über Limit gehalten → Ziel 275 (+8,8 %, positiv). Outcomes gemischt → kein klarer SKILL; braucht KONSISTENTE Verlängerungsregel (Wed-SKILL-Urteil war voreilig, L10). |
